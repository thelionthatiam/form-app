    ...
    db.query('SELECT * FROM cart_items WHERE cart_uuid = $1', [req.session.user.cart_uuid])
      .then((result) => {
        if (result.rowCount === 0) {
          ...
          return db.query('SELECT * FROM orders WHERE cart_uuid = $1', [req.session.user.cart_uuid])
        }
      })
      .then((result) => {
        let number = result.rows.length;
        numberOfOrders = number + 1;
        return db.query('INSERT INTO orders (cart_uuid, card_number, order_number) VALUES ($1, $2, $3)', [req.session.user.cart_uuid, card_number, numberOfOrders])
      })
      .then((result) => {
        return db.query('SELECT order_uuid FROM orders WHERE cart_uuid = $1 AND order_number = $2', [req.session.user.cart_uuid, numberOfOrders])
      })
      .then((result) => {
