...
router.route('/orders')
  .post((req, res) => {
    ...
    let order_uuid = '';
    db.query('INSERT INTO orders (cart_uuid, card_number) VALUES ($1, $2)', [req.session.user.cart_uuid, card_number])
      .then((result) => {
        console.log('result of insert into orders', result);
        return db.query('SELECT order_uuid FROM orders WHERE cart_uuid = $1', [req.session.user.cart_uuid])
      })
      .then((result) => {
