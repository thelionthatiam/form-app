    ...
    db.query(query, values)
      .then((result) => {
        if (result.rows.length === 0) {
          return db.query('INSERT INTO cart(product_id, name, price, size, user_uuid, quantity) VALUES ($1, $2, $3, $4, $5, $6)', [inputs.product_id, inputs.name, inputs.price, inputs.size, inputs.uuid, inputs.quantity])
        } else {
  ...
  .get((req, res) => {
    ...
    db.query(query, values)
      .then((result) => {
        ...
        return db.query('SELECT card_number FROM payment_credit WHERE (user_uuid, active) = ($1, $2)', [uuid, true])
      })
      .then((result) => {
