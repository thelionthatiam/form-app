...
router.route('/cart')
    ...
    async_database_1.db.query('SELECT card_number FROM payment_credit WHERE (user_uuid, active) = ($1, $2)', [inputs.uuid, true])
        ...
        return async_database_1.db.query('SELECT product_history_id FROM product_history p WHERE updated_timestamp = (SELECT MAX(updated_timestamp) FROM product_history WHERE product_id = $1)', [inputs.product_id]);
    })
        .then((result) => {
        ...
        if (result.rows.length === 0) {
            async_database_1.db.query('SELECT cc.coupon_uuid, discount, applies_to, applied FROM cart_coupons cc INNER JOIN coupons c ON c.coupon_uuid = cc.coupon_uuid AND (cc.cart_uuid = $1)', [req.session.user.cart_uuid])
                .then((result) => {
    ...
    .get((req, res) => {
    let uuid = req.session.user.uuid, cartContent = [], totalCost = 0, totalItems = 0, price, quantity, card_number, lastFour, discounted = 1;
