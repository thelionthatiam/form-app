...
class Alarm {
    constructor(user_uuid, title, active, alarm_uuid, state, time) {
    ...
    addSnooze() {
        ...
        database_1.db.query(query, input)
            .then((result) => {
            return database_1.db.query('INSERT INTO snoozes(user_uuid, alarm_uuid) VALUES ($1, $2)', [this.user_uuid, this.alarm_uuid]);
        })
            .catch((error) => {
            console.log(error);
        });
    ...
    addDismiss() {
        ...
        database_1.db.query(query, input)
            .then((result) => {
            return database_1.db.query('INSERT INTO dismisses(user_uuid, alarm_uuid) VALUES ($1, $2)', [this.user_uuid, this.alarm_uuid]);
        })
            .catch((error) => {
            console.log(error);
        });
    ...
    addWake() {
        ...
        database_1.db.query(query, input)
            .then((result) => {
            return database_1.db.query('INSERT INTO wakes(user_uuid, alarm_uuid) VALUES ($1, $2)', [this.user_uuid, this.alarm_uuid]);
        })
            .catch((error) => {
            console.log(error);
        });
    ...
    snoozing() {
        ...
        let thing = setInterval(() => {
            ...
            database_1.db.query('SELECT state FROM alarms WHERE user_uuid = $1 AND alarm_uuid = $2', [this.user_uuid, this.alarm_uuid])
                .then((result) => {
